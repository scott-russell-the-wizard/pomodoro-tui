...
class HelpModal(ModalScreen[None]):
    ...
    def compose(self) -> ComposeResult:
        with Container(id="help_dialog"):
            ...
            yield Static("  [bold]a[/bold]            Quick Add Task dialog")
            yield Static("  [bold]1[/bold]            Switch to Timer tab")
            yield Static("  [bold]2[/bold] or [bold]t[/bold]        Switch to Tasks tab (focus input)")
            ...
            yield Static("Task Management & Tracking", classes="help_section_title")
            yield Static("  • Press [bold]a[/bold] from anywhere to quickly add a task and set it active.")
            yield Static("  • On the Tasks tab ([bold]t[/bold]), press [bold]Enter[/bold] on any task row to set active.")
            yield Static("  • When a focus session finishes, your active task gets +1 🍅!")

            yield Static("Audio & Notifications over SSH", classes="help_section_title")
            yield Static("  • Sessions trigger 3 terminal bell pulses (\\07) and OSC 9/777 alerts.")
            yield Static("  • In macOS Terminal / iTerm2, ensure 'Audible bell' is enabled in settings.")
            yield Static("  • Modern terminals will also show a desktop toast via OSC escape codes.")
